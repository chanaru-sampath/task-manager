...
describe('TaskForm', () => {
  ...
  it('add mode: typing title, filling date, and submitting calls addTask and toasts', async () => {
    ...
    expect(tasks[0].dueOn).toBe('2026-12-31')
    expect(toast.success).toHaveBeenCalledWith('Task added')
  })

  it('add mode: empty title shows a validation error and does not add', async () => {
  ...
  it('edit mode: submitting calls updateTask (and allows past dates)', async () => {
    ...
    // Past date is allowed in edit mode; submit the pre-filled form
  ...
  it('cancel button calls onOpenChange(false) without mutating store', async () => {
